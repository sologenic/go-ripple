...
The rippled trades should use the ripple_state nodes to show the full trade path.
function newRippleState is a first attempt to structure this data, however for now a different solution is used in the code to collapse the offers in certain scenarios into a single line.
This single line, if it works well (more records need to be analyzed to see if it's correct), will be ported into the newTrade function once verified, however it verification fails,
the newRippleState function work will be usable.
...
func newRippleState(txm *TransactionWithMetaData, i int) (*Trade, error) {
	...
	paid.Issuer = v.LowLimit.Issuer
	got := v.Balance
	got.Issuer = v.LowLimit.Issuer
	if p.Balance != nil {
		var err error
		got, err = p.Balance.Subtract(v.Balance)
		if err != nil {
			return nil, err
		}
		got = got.Abs()
